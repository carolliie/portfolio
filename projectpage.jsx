/* global React, PROJECTS, Dither, Glitch, Magnetic */
  ...
  if (!p) {
    return (
      <div className="pp wrap">
        ...
        <button className="btn btn--ghost crt-type" onClick={() => onNav("projetos")} data-cursor="link">← voltar</button>
      </div>
    );
  }
  ...
  return (
    <article className="pp">
      <div className="pp__hero">
        <Dither mode="scape" seed={p.seed + 0.5} cols={240} rows={96} className="pp__herodither" />
        <div className="pp__heroveil" />
        <div className="pp__herotop wrap crt-type">
          <button className="pp__back" onClick={() => onNav("projetos")} data-cursor="link">← projetos</button>
          <span>{p.year} · {p.kind}</span>
        </div>
        <div className="pp__herotitle wrap">
          <Glitch tag="h1" className="pp__name">{p.name}</Glitch>
        ...
        <div className="pp__main">
          ...
          <section className="pp__block reveal">
            ...
            <ul className="pp__features">
              {p.features.map((f, i) => (
                <li key={i}><span className="pp__fnum crt-type">{String(i + 1).padStart(2, "0")}</span>{f}</li>
              ))}
            </ul>
          </section>
        </div>
        ...
        <aside className="pp__side reveal">
          ...
          <div className="pp__panel">
            ...
            <div className="stack__chips">
              {p.stack.map((s) => <span className="chip crt-type" key={s}>{s}</span>)}
            </div>
          </div>
          <Magnetic>
            <a className="btn btn--solid crt-type pp__visit" href="#" onClick={(e) => e.preventDefault()} data-cursor="link">
              ver projeto ao vivo →
            </a>
          </Magnetic>
          <span className="pp__note crt-type">link em breve</span>
        </aside>
      </div>

      <div className="pp__next wrap" onClick={() => onOpen(next.id)} data-cursor="view">
        <span className="pp__nextlbl crt-type">próxima layer →</span>
        <Glitch tag="h2" className="pp__nextname">{next.name}</Glitch>
      </div>
    </article>
  );
}
